...
if (empty($BlogsInstanciada)) {
	...
	class Blogs
	{
		...
		function rsDados($id = '', $orderBy = '', $limite = '', $principal = '', $destaque = '', $catDestaque = '', $categoria = '', $url_amigavel = '')
		{
			...
			if (!empty($catDestaque)) {
				$sql .= " and cat_destaque = ?";
				$nCampos++;
				$campo[$nCampos] = $catDestaque;
			}
			if (!empty($categoria)) {
		...
		function noticiasPopulares($id = '', $orderBy = '', $limite = '')
		{
			...
			try {
				$sql = "SELECT tbl_blog.*, tbl_categoria.nome AS nomeCategoria 
						FROM tbl_blog 
						LEFT JOIN tbl_categoria ON tbl_categoria.id = tbl_blog.id_categoria
						WHERE 1=1 AND tbl_blog.principal = 'N' AND tbl_blog.destaque = 'N' $sqlOrdem $sqlLimite";
				$stm = $this->pdo->prepare($sql);
		...
		function add($redireciona = '')
		{
			if (isset($_POST['acao']) && $_POST['acao'] == 'addBlog') {
				...
				$idCategoria = filter_input(INPUT_POST, 'categoria', FILTER_SANITIZE_SPECIAL_CHARS);
				...
				try {
					...
					$sql = "INSERT INTO tbl_blog (foto, titulo, postado_por, conteudo, breve, meta_title, meta_keywords, meta_description, foto1, url_amigavel, descricao_imagem, legenda_imagem, ativo, principal, destaque, audio, pdf, embed, id_categoria) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
					$stm = $this->pdo->prepare($sql);
				...
				try {
					echo $sql = "SELECT id, id_categoria FROM tbl_blog WHERE 1=1 and cat_destaque = 'S';";
					$stm = $this->pdo->prepare($sql);
					$stm->execute();
					$rsCatDestaques = $stm->fetchAll(PDO::FETCH_OBJ);


					if (isset($rsCatDestaques) && $rsCatDestaques[0]->id == $ultimoPost) {
						echo $sql = "UPDATE tbl_blog SET cat_destaque = 'N', principal = 'S' WHERE id_categoria != {$rsCatDestaques[0]->id_categoria}";
						$stm = $this->pdo->prepare($sql);
						$stm->execute();
					}
				} catch (PDOException $erro) {
		...
		function editar($redireciona = 'noticias.php')
		{
			if (isset($_POST['acao']) && $_POST['acao'] == 'editaBlog') {
				...
				try {
					...
					$sql = "UPDATE tbl_blog SET foto=?, titulo=?, postado_por=?, conteudo=?, breve=?, meta_title=?, meta_keywords=?, meta_description=?, foto1=?, url_amigavel=?, descricao_imagem=?, legenda_imagem=?, ativo=?, principal=?, destaque=?, audio=?, pdf=?, embed=?, id_categoria=? WHERE id=?";
					$stm = $this->pdo->prepare($sql);
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));
					$stm->bindValue(2, $titulo);
					$stm->bindValue(3, $postado_por);
